...
if [[ ${ALL_DATABASES} == "" ]]; then
	...
	mysqldump --user="${DB_USER}" --password="${DB_PASS}" --host="${DB_HOST}" "$@" "${DB_NAME}" | gzip -c | cat > /mysqldump/"${DB_NAME}"_${TS}.sql
else
...
for db in $databases; do
    if [[ "$db" != "information_schema" ]] && [[ "$db" != "performance_schema" ]] && [[ "$db" != "mysql" ]] && [[ "$db" != _* ]] && [[ "$db" != "$IGNORE_DATABASE" ]]; then
        ...
        mysqldump --user="${DB_USER}" --password="${DB_PASS}" --host="${DB_HOST}" --databases $db | gzip -c | cat > /mysqldump/$db-${TS}.sql
    fi
done
fi
